...
namespace OwnApt.Common.Security
{
    public static class RsaCryptoServiceProviderExtensions
    {
        ...
        private static Dictionary<string, byte[]> ParseRsaXmlString(string rsaXmlString)
        {
            ...
            {
                rsaXmlString = rsaXmlString.Substring(rsaXmlString.IndexOf($"<{tagName}>", StringComparison.CurrentCulture) + $"<{tagName}>".Length);
                rsaPropertyCache.Add(tagName, Convert.FromBase64String(rsaXmlString.Substring(0, rsaXmlString.IndexOf($"</{tagName}>", StringComparison.CurrentCulture))));
            }
